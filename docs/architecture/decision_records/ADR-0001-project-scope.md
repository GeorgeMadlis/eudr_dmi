...
Historically, this repository’s documentation used wording that implied a read-only, “consume-only” relationship to an upstream platform (`geospatial_dmi`). That framing is too restrictive for an audit-driven program that must:
...
## Consequences
- No “upstream read-only” constraint: this repo may incorporate and modify adopted components as needed.
- An explicit adoption/provenance log becomes part of change control.
- Divergence is allowed (and expected) when required; compatibility with `geospatial_dmi` is not assumed unless explicitly documented.
